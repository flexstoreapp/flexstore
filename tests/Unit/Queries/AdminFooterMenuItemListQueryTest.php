...
covers(AdminFooterMenuItemListQuery::class);

uses()->group('queries', 'menu-item');

test('returns footer menu items', function () {
    MenuItem::factory()->footer()->create(['parent_id' => null]);
    MenuItem::factory()->header()->create(['parent_id' => null]);

    $query = new AdminFooterMenuItemListQuery();
...
test('returns only root items', function () {
    $parent = MenuItem::factory()->footer()->create(['parent_id' => null]);
    MenuItem::factory()->footer()->create(['parent_id' => $parent->id]);
...
test('includes both active and inactive items', function () {
    MenuItem::factory()->footer()->active()->create(['parent_id' => null]);
    MenuItem::factory()->footer()->inactive()->create(['parent_id' => null]);
...
test('orders items by sort_order', function () {
    MenuItem::factory()->footer()->create(['parent_id' => null, 'sort_order' => 2, 'label' => 'Second']);
    MenuItem::factory()->footer()->create(['parent_id' => null, 'sort_order' => 1, 'label' => 'First']);
...
test('returns empty collection when no footer items exist', function () {
    MenuItem::factory()->header()->create(['parent_id' => null]);
